use super::instance_action::{PodContext, InstanceAction, create_pod_context, do_pod_action_for_nodes, handle_instance_change_job};
use akri_shared::{
    akri::configuration::{BrokerType, Configuration},
    akri::instance::Instance,
    k8s,
    k8s::{KubeInterface, pod},
};
use futures::{StreamExt, TryStreamExt};
use kube::api::{Api, ObjectList, ListParams};
...
async fn get_entry(config_name: &str, config_map: ConfigMap) -> Option<i32> {
    config_map.read().await.get(config_name).unwrap_or(&None).clone()
}
    ...
    match event {
        Event::Applied(config) => {
            ...
            match &config.spec.deployment_strategy {
                    ...
                    match get_entry(config.metadata.name.as_ref().unwrap(), config_map.clone()).await {
                        Some(last_broker_generation) => {
                            if last_broker_generation < current_gen {
                                config_map
                                .write()
                                .await.insert(config.metadata.name.as_ref().unwrap().to_string(), Some(current_gen));
                                match &deployment.broker_type {
                                    BrokerType::Pod(p) => {
                                        handle_broker_updates_for_configuration_pod(kube_interface, &config, &p).await?;
                                    }
                                    BrokerType::Job(j) => {
                                        handle_broker_updates_for_configuration_job(kube_interface, &config, &j).await?;
                                    }
                                }
                            }
                        },    
                ...
                None => {
                    config_map.write().await.insert(config.metadata.name.as_ref().unwrap().to_string(), None);
                }
            }
        ...
        Event::Deleted(config) => {
            ...
            config_map
            .write()
            .await.remove(config.metadata.name.as_ref().unwrap());
        }
...
async fn handle_broker_updates_for_configuration_pod(
    ...
    kube_interface
    .find_pods_with_label(&format!("{}={}", pod::AKRI_CONFIGURATION_LABEL_NAME, config.metadata.name.as_ref().unwrap()))
    .await?.into_iter().try_for_each(|p| -> anyhow::Result<()> {
        let instance_name = p.metadata.labels.as_ref().unwrap().get(pod::AKRI_INSTANCE_LABEL_NAME).as_ref().ok_or_else(|| {
            anyhow::anyhow!("No instance label found for Pod {}", p.metadata.name.as_ref().unwrap())
        })?.clone();
        let pod_name = p.metadata.name.as_ref().unwrap().to_string();
        let c = create_pod_context(&p)?;
        configuration_pods.entry(instance_name.to_string()).and_modify(|m| {
    ...
    .find_instances_with_label(Some(format!("{}={}", pod::AKRI_CONFIGURATION_LABEL_NAME, config.metadata.name.as_ref().unwrap())), config.metadata.namespace.as_ref().unwrap())
    .await?.into_iter().map(|i| (i.metadata.name.as_ref().unwrap().to_string(), i)).collect();
    // For each instance, do pod action for nodes
    let futures: Vec<_> = configuration_pods.into_iter().filter_map( |(i_name, m)|
        if let Some(instance) =  instances.get(&i_name) {
            // Create pod context for each Pod
            Some(do_pod_action_for_nodes(m, instance, podspec, kube_interface))
        } else {
            None
        }
    ).collect();
...
async fn handle_broker_updates_for_configuration_job(
    ...
    .find_instances_with_label(Some(format!("{}={}", pod::AKRI_CONFIGURATION_LABEL_NAME, config.metadata.name.as_ref().unwrap())), namespace)
    .await?;
    trace!("handle_broker_updates_for_configuration_job - after finding instances {:?}", instances);
    ...
    kube_interface.delete_jobs_with_label(Some(format!("{}={}", pod::AKRI_CONFIGURATION_LABEL_NAME, config.metadata.name.as_ref().unwrap())), namespace).await?;
    ...
    for i in instances {
        handle_instance_change_job(i, job_spec, &InstanceAction::Add, kube_interface).await?;
    }
...
mod config_action_tests {
    ...
    async fn test_handle_config_blah() {
        ...
        mock.expect_find_instances_with_label().times(1).returning(move |_,_| {
        ...
        mock.expect_delete_jobs_with_label().times(1).returning(|_,_| {Ok(())});
        handle_config(&mock, Event::Applied(config), Arc::new(RwLock::new(config_map)), &mut false).await.unwrap();
    }
