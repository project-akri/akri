...
pub mod registration_server {
    ...
    pub const SERVICE_NAME: &str = "v0.Registration";
...
pub mod discovery_handler_server {
    ...
    #[async_trait]
    pub trait DiscoveryHandler: std::marker::Send + std::marker::Sync + 'static {
    ...
    #[derive(Debug)]
    pub struct DiscoveryHandlerServer<T> {
        inner: Arc<T>,
        accept_compression_encodings: EnabledCompressionEncodings,
        send_compression_encodings: EnabledCompressionEncodings,
        max_decoding_message_size: Option<usize>,
    ...
    impl<T> DiscoveryHandlerServer<T> {
        pub fn new(inner: T) -> Self {
            Self::from_arc(Arc::new(inner))
        }
        pub fn from_arc(inner: Arc<T>) -> Self {
    ...
    impl<T> Clone for DiscoveryHandlerServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    /// Generated gRPC service name
    pub const SERVICE_NAME: &str = "v0.DiscoveryHandler";
    impl<T> tonic::server::NamedService for DiscoveryHandlerServer<T> {
        const NAME: &'static str = SERVICE_NAME;
    }
}
