...
def initial_sleep():
    ...
    if os.path.exists(SLEEP_DURATION_FILE):
        initial_sleep_duration = open(SLEEP_DURATION_FILE, "r").readline().rstrip()
...
def check_pods_running(v1, pod_label_selector, count):
    print("Checking number of pods [{}] ... expected {}".format(pod_label_selector, count))
    for x in range(30):
        if x != 0:
            time.sleep(SLEEP_INTERVAL)
            print("Sleep iteration {} ... been waiting for {} seconds for pod check".format(x+1, (x+1)*SLEEP_INTERVAL))
        pods = v1.list_pod_for_all_namespaces(label_selector=pod_label_selector).items
    ...
    print("Wrong number of pods [{}] found ... expected {}".format(pod_label_selector, count))
    return False

def check_svcs_running(v1, svc_label_selector, count):
    print("Checking number of svcs  [{}] ... expected {}".format(svc_label_selector, count))
    for x in range(30):
        if x != 0:
            time.sleep(SLEEP_INTERVAL)
            print("Sleep iteration {} ... been waiting for {} seconds for svc check".format(x+1, (x+1)*SLEEP_INTERVAL))
        svcs = v1.list_service_for_all_namespaces(label_selector=svc_label_selector).items
...
def check_instance_count(count):
    print("Checking for instances ... version:{} count:{}".format(major_version, count))
    api_instance = client.CustomObjectsApi()
    for x in range(20):
        if x != 0:
            time.sleep(SLEEP_INTERVAL)
        print("Sleep iteration {} ... been waiting for {} seconds for instances".format(x+1, (x+1)*SLEEP_INTERVAL))
        instances = api_instance.list_namespaced_custom_object(GROUP, major_version, "default", "instances")['items']
...
def check_agent_pods_state(v1, agents):
    ...
    agents_check_failed = check_pods_running(v1, AGENT_POD_LABEL_SELECTOR, agents)
...
def check_controller_pods_state(v1, controllers):
    ...
    controllers_check_failed = check_pods_running(v1, CONTROLLER_POD_LABEL_SELECTOR, controllers)
    if not controllers_check_failed:
        print("Wrong number of controllers found ... expected {}".format(controllers))
    else:
        if controllers == 1:
            controller_pod_name = get_pod_name(CONTROLLER_POD_LABEL_SELECTOR, 0)
...
def check_broker_pods_state(v1, brokers):
    ...
    brokers_check_failed = check_pods_running(v1, BROKER_POD_LABEL_SELECTOR, brokers)
    if not brokers_check_failed:
        print("Wrong number of brokers found ... expected {}".format(brokers))
    return brokers_check_failed

def check_config_svcs_state(v1, count):
    print("Checking for configuration services ... expected {}".format(count))
    config_svcs_check_failed = check_svcs_running(v1, CONFIGURATION_SVC_LABEL_SELECTOR, count)
    if not config_svcs_check_failed:
        print("Wrong number of configuration services found ... expected {}".format(count))
    return config_svcs_check_failed

def check_instance_svcs_state(v1, count):
    print("Checking for instance services ... expected {}".format(count))
    instance_svcs_check_failed = check_svcs_running(v1, INSTANCE_SVC_LABEL_SELECTOR, count)
...
def check_akri_state(agents, controllers, instances, brokers, config_svcs, instance_svcs):
    print("Checking for Akri state ... expected agent(s):{}, controller(s):{}, instance(s):{}, broker(s):{}, config service(s):{}, and instance service(s):{} to exist".format(agents, controllers, instances, brokers, config_svcs, instance_svcs))
    v1 = client.CoreV1Api()
    return check_agent_pods_state(v1, agents) and \
           check_controller_pods_state(v1, controllers) and \
           check_instance_count(instances) and \
